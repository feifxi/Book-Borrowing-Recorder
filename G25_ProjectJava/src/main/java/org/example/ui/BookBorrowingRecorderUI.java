...
import org.example.repository.database.DBBookRepository;
import org.example.repository.database.DBBorrowRecordRepository;
import org.example.repository.database.DBCustomerRepository;
import org.example.repository.database.DatabaseConnection;
import org.example.repository.file.FileBookRepository;
import org.example.repository.file.FileBorrowRecordRepository;
import org.example.repository.file.FileCustomerRepository;
import org.example.repository.memory.InMemoryBookRepository;
import org.example.repository.memory.InMemoryBorrowRecordRepository;
import org.example.repository.memory.InMemoryCustomerRepository;
import org.example.service.BookBorrowingRecorder;

public class BookBorrowingRecorderUI {
    private BookBorrowingRecorder service;
    private Scanner sc = new Scanner(System.in);
    private UIComponent uiComponent;

    public BookBorrowingRecorderUI(){     // Default is In-Memory Storage
        service = new BookBorrowingRecorder(
    ...
    public void chooseStorage() {
        ...
        while (true) {
            uiComponent.welcome();
            uiComponent.chooseStorage();
            uiComponent.showStatusMessage();
            System.out.print("Choose [1|2|3] :");
            storage = sc.nextLine();
            if (storage.isBlank()) continue;
            else if (storage.equalsIgnoreCase("q")) return;
            else if (storage.equals("1")) break;
            else if (storage.equals("2")) break;
            else if (storage.equals("3")) {
                if (DatabaseConnection.isConnecting()) break;
                uiComponent.setStatusMessage("*Please connecting to database and try again");
            }
        }

        // Set Storage
        if (storage.equals("1")){         // In-Memory
            this.service = new BookBorrowingRecorder(
                    new InMemoryBookRepository(),
                    new InMemoryCustomerRepository(),
                    new InMemoryBorrowRecordRepository());
        }
        else if (storage.equals("2")){       // File
            this.service = new BookBorrowingRecorder(
                    new FileBookRepository(),
                    new FileCustomerRepository(),
                    new FileBorrowRecordRepository());
        }
        else if (storage.equals("3")){      // Database
            this.service = new BookBorrowingRecorder(
                    new DBBookRepository(),
                    new DBCustomerRepository(),
                    new DBBorrowRecordRepository());
        }
        this.uiComponent = new UIComponent(service);
        loginUI();
    }
    ...
    public void bookMenuUI(){
        while (true) {
            ...
            else if (choose.equals("3")) {      // List all book
                uiComponent.setBooksListingOption("AllBook");
            }
            else if (choose.equals("4")) {      // List book by title
                while (true) {
                    // Display
                    uiComponent.bookHeader("Menu", null);
                    uiComponent.listAllBooks();
            ...
            else if (choose.equals("5")) {      // List book by author
                while (true) {
                    // Display
                    uiComponent.bookHeader("Menu", null);
                    uiComponent.listAllBooks();
    ...
    public void addOrUpdateCustomerUI(String option, String id){
        ...
        while (true) {
            while (true) {     // get Name
                ...
                if (customer != null) { // check duplicate customer name
